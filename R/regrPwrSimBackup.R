...
regrPwrSim <- function(n, predictors=NULL, cor = c(.3, .5), 
  ...
  res$intermediate$regressionAnalyses <-
    do(samples) * function(mu = rep(means, (predictors+1)),
      ...
      for (currentPredictor in predictNames) {
        ...
        res$correctedCorrelations[[currentPredictor]]$cor.pred_corr_dep <-
      ...
      res <- list(significances = tail(summary(regressionOutcome)$coefficients[,'Pr(>|t|)'], -1),
                  correlations = cor(dat),
                  cor.pred_raw_dep =
                    unlist(lapply(res$correctedCorrelations, function(x) {
                      return(x$cor.pred_raw_dep);
                    })),
                  cor.pred_corr_dep =
                    unlist(lapply(res$correctedCorrelations, function(x) {
                      return(x$cor.pred_corr_dep);
                    })));
  ...
  ### print(res$intermediate$regressionAnalyses$significances);
  #print(res$intermediate$regressionAnalyses$cor.pred_raw_dep);
  #print(res$intermediate$regressionAnalyses$significances);
  ...
  ### Convert corrected correlations to dataframe
  res$intermediate$regressionAnalyses$cor.pred_raw_dep <-
    t(data.frame(res$intermediate$regressionAnalyses$cor.pred_raw_dep));
  row.names(res$intermediate$regressionAnalyses$cor.pred_raw_dep) <- NULL;
  res$intermediate$regressionAnalyses$cor.pred_corr_dep <-
    t(data.frame(res$intermediate$regressionAnalyses$cor.pred_corr_dep));
  row.names(res$intermediate$regressionAnalyses$cor.pred_corr_dep) <- NULL;
  ...
  ### Convert correlations corrected for predictors to Fisher's Z's
  res$intermediate$corrRaw_fishersZ <-
    t(apply(res$intermediate$regressionAnalyses$cor.pred_raw_dep, 1, fisherz));
  res$intermediate$corrRaw_meanFishersZ <-
    colMeans(res$intermediate$corrRaw_fishersZ);
  res$intermediate$corrRaw_generatedCorrelations <-
    fisherz2r(res$intermediate$corrRaw_meanFishersZ);

  ### Convert correlations with corrected dependent to Fisher's Z's
  res$intermediate$corrCorr_fishersZ <-
    t(apply(res$intermediate$regressionAnalyses$cor.pred_corr_dep, 1, fisherz));
  res$intermediate$corrCorr_meanFishersZ <-
    colMeans(res$intermediate$corrCorr_fishersZ);
  res$intermediate$corrCorr_generatedCorrelations <-
    fisherz2r(res$intermediate$corrCorr_meanFishersZ);
  
  ### Set diagonal for generated correlations to 1 (fisherz2r returns them
  ### at NA)
  ...
  res$intermediate$bivariatePower.semipartial <-
    sapply(res$intermediate$corrRaw_generatedCorrelations,
           function(x, n) {
             return(pwr.r.test(r=x, n=n, sig.level=sig.level)$power);
           }, n=n);
  
  res$intermediate$bivariatePower.partial <-
    sapply(res$intermediate$corrCorr_generatedCorrelations,
           function(x, n) {
             return(pwr.r.test(r=x, n=n, sig.level=sig.level)$power);
           }, n=n);
  ...
  res$output$dat <- data.frame(bivar.cor = res$intermediate$generatedCorrelations[c(predictorNames, names(interactions)), dependentName],
                               ...
                               cor.sp = res$intermediate$corrRaw_generatedCorrelations,
                               cor.partial = res$intermediate$corrCorr_generatedCorrelations,
                               pwr.sp = res$intermediate$bivariatePower.semipartial,
                               pwr.partial = res$intermediate$bivariatePower.partial);
