confidenceCurve <- function(metric = 'd', value = .5, n = 128,
  ...
  if (tolower(metric) == 'd') {
    if (is.null(n)) {
      ...
      names(df1) <- names(df2) <- c('n', 'd');
      plot <- ggplot() +
        geom_line(data=df1, aes_string(x='d', y='n')) +
        geom_line(data=df2, aes_string(x='d', y='n')) +
      ...
      df1 <- cbind(df1, confInts[, 1]);
      df2 <- cbind(df2, confInts[, 2]);
      names(df1) <- names(df2) <- c('confidence', 'd');
      plot <- ggplot() +
        geom_line(data=df1, aes_string(x='d', y='confidence')) +
        geom_line(data=df2, aes_string(x='d', y='confidence')) +
